...
-- Enable Row Level Security (RLS) for all tables (optional but recommended)
...
CREATE POLICY "Users can create events" ON events FOR INSERT WITH CHECK (auth.uid()::text = host_id);

-- Allow hosts to update their own events
CREATE POLICY "Hosts can update own events" ON events FOR UPDATE USING (auth.uid()::text = host_id);
...
USING (
  ...
  auth.uid()::text IN (SELECT host_id FROM events WHERE id = event_id)
);
...
USING (
  ...
  auth.uid()::text IN (SELECT host_id FROM events WHERE id = event_id) OR
  auth.uid()::text IN (SELECT user_id FROM bookings WHERE event_id = chat_messages.event_id)
);
