...
function createClusterIcon(count: number) {
  return divIcon({
    html: `<div class="custom-cluster-icon"><span class="cluster-inner">${count}</span></div>`,
...
// Create event icon based on sport
...
export default function MapView({ events, userLocation, onEventClick, onJoin, onOpenChat, onCancel, onModify, currentUserId }: MapViewProps) {
  ...
  const eventClusters = useMemo(() => {
    ...
    events.forEach(event => {
      ...
      events.forEach(otherEvent => {
        if (processed.has(otherEvent.id) || otherEvent.id === event.id || !otherEvent.latitude || !otherEvent.longitude) return;
        ...
        const distance = Math.sqrt(
          Math.pow((eventLat - otherLat) * 111000, 2) + 
          Math.pow((eventLng - otherLng) * 111000 * Math.cos(eventLat * Math.PI / 180), 2)
        );
  ...
  const mapCenter: [number, number] = useMemo(() => {
    ...
    return [40.1164, -88.2434]; // Default center (Champaign, IL)
  }, [events, userLocation]);
                    ...
                    <div className="w-full">
                      <div className="p-4">
                        <div className="flex items-start justify-between mb-3">
                          <div>
                            ...
                            <p className="text-sm text-gray-600 mt-1">
                              by {cluster.events[0].host.email?.split('@')[0] || 'Host'}
                            </p>
                          </div>
                        ...
                        <div className="space-y-2 mb-4">
                          ...
                          <div className="flex items-center gap-2 text-sm text-gray-600">
                            <Users className="w-4 h-4" />
                            {cluster.events[0].currentPlayers || 0} / {cluster.events[0].maxPlayers} players
                          </div>
